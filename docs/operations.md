...
`cinder-move-engine-stack` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies engine behavior through fixture event logs, with golden state snapshots and explicit failure cases.
...
The score is deterministic. The current threshold is 160. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 3.
